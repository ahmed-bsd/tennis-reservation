    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="space-y-6">
        {Object.entries(byCourt).map(([key, courtSlots]) => {
          const { courtNumber, courtName } = courtSlots[0]

          return (
            <div key={key} className="card">
              <h2 className="font-semibold text-gray-700 mb-3">
                Court {courtNumber} — {courtName}
              </h2>
              ...
              <div className="flex flex-wrap gap-2">
                {courtSlots.map(slot => {
                  ...
                  const hasDiscount =
                    slot.originalPrice && slot.originalPrice > slot.price

                  const discountPercent = hasDiscount
                    ? Math.round(
                        ((slot.originalPrice - slot.price) /
                          slot.originalPrice) * 100
                      )
                    : 0
                  ...
                  return (
                    <button
                      ...
                      className={`relative px-3 py-2 rounded-lg text-sm font-medium border transition-all duration-200
                        ${!slot.available
                          ? 'bg-red-50 border-red-200 text-red-400 cursor-not-allowed line-through'
                          : isSelected
                            ? 'bg-tennis-green border-tennis-green text-white'
                            : hasDiscount
                              ? 'bg-gradient-to-r from-yellow-50 via-orange-50 to-yellow-100 border-orange-300 text-gray-800 shadow-md hover:scale-105'
                    ...
                    >
                      ...
                      {/* DISCOUNT BADGE */}
                      {hasDiscount && slot.available && (
                        <div className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full shadow">
                          -{discountPercent}%
                        </div>
                      )}
      ...
      {selected && (
        <div className="fixed bottom-6 right-6 card shadow-xl border-tennis-green border-2 w-80">
          ...
          <p className="text-sm text-gray-600">
            Court {selected.courtNumber} — {selected.courtName}
          </p>

          <p className="text-sm text-gray-600">
            {date} at {selected.startTime.slice(0,5)} · {duration} min
          </p>

          <p className="text-sm font-medium text-tennis-green mt-1">
            Price per 30min: {selected.price} jetons
          </p>
